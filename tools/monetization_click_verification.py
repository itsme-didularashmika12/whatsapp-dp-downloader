...
check("ads.js contains NO preventDefault (native navigation never blocked)", "preventDefault" not in adsjs)
check("ads.js contains no window.open / no auto-open path", "window.open" not in adsjs)
...
with sync_playwright() as p:
    ...
    for page_i, rot_off in enumerate(p for _ in range(3) for p in (0, 2, 4, 6)):
        ...
        anchors = pg.locator("a.sponsored-link")
        n = anchors.count()
        for i in range(n):
            a = anchors.nth(i)
